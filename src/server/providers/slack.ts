...
function slackAccessTokenKey(providerId: string): string {
  return `slack:${providerId}:access_token`;
}
...
function slackNeedsAuthURL(providerID: string): string {
  return `/api/oauth/slack?provider_id=${encodeURIComponent(providerID)}`;
}
...
export class SlackProvider implements Provider<SlackProviderArgs> {
  type: string;
  id: string;

  constructor(config: ProviderConfig<SlackProviderArgs>) {
    this.type = config.type;
    this.id = config.id;
  }

  authInitURL(_args: SlackProviderArgs, baseURL: string): string {
  ...
  handleAuthCallback(accessToken: string, secrets: SecretStore): void {
    secrets.set(slackAccessTokenKey(this.id), accessToken);
  }
  ...
  async fetchConvos(
    args: SlackProviderArgs,
    secrets: SecretStore,
  ): Promise<FetchConvosResult> {
    const accessToken = secrets.get(slackAccessTokenKey(this.id));
    if (!accessToken) {
      return {
        convos: [],
        needsAuth: {
    ...
    if (!searchQuery) {
      throw new Error('Slack provider args.searchQuery is required.');
    }
    ...
    try {
      ...
      if (
        ...
        secrets.delete(slackAccessTokenKey(this.id));
        return {
          convos: [],
          needsAuth: {
            url: slackNeedsAuthURL(this.id),
